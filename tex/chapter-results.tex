...
\Cref{tab:track_loss_improvement} shows the improvement in track loss of the different AIS scenarios compared to the pure radar scenario. 
...
\Cref{fig:scenario0_tracking_percentage,fig:init_performance_scenario1,fig:scenario2_tracking_percentage,fig:scenario3_tracking_percentage,fig:init_performance_scenario4} show a substantial increase in tracking percentage for low \gls{Pd} and windows size when comparing the pure radar tracking with the \gls{ais} aided tracking. The difference is biggest for N=1 in all cases, but since this is an very small window size, our analysis will focus on N=3 and 6 since these seems to be the sweet spot between runtime (\Cref{fig:scenario0_tracking_runtime,fig:scenario1_tracking_runtime,fig:scenario2_tracking_runtime,fig:scenario3_tracking_runtime,fig:scenario4_tracking_runtime}) and performance. 

\Cref{tab:tracking_percentage_improvement} list the improvements for the AIS scenarios compared to the baseline without AIS, all for \(\lambda_\phi = 1\cdot 10^{-5}\). From this we can see that the class of AIS equipment impacts the tracking performance more than the amount of AIS transmitters. It also shows that the gain of AIS reduces with an increased \gls{Pd} and N.
